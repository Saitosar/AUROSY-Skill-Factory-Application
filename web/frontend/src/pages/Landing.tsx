...
const CODE_LINES = [
  { text: "import mujoco, numpy as np, torch, onnxruntime as ort", color: "text-[#c084fc]" },
  { text: "from aurosy.skills import SkillFactory, deploy_config, PolicyRunner", color: "text-[#e879f9]" },
  { text: "from unitree_sdk2.g1 import G1Robot, JointState, IMUSensor, Actuator", color: "text-[#c084fc]" },
  { text: "", color: "" },
  { text: "class BalanceSkill(SkillFactory.BaseSkill, metaclass=SkillMeta):", color: "text-[#22d3ee]" },
  { text: "    def __init__(self, robot_model='g1_29dof', sim_dt=0.002, decimation=10):", color: "text-[#fbbf24]" },
  { text: "        self.kp, self.kd = 120.0, 10.0  # PD gains for joint position control", color: "text-[#4ade80]" },
  { text: "        self.target_height = 0.75  # target CoM height in meters from ground plane", color: "text-[#34d399]" },
  { text: "        self.joint_limits = robot_model.get_limits(safety_margin=0.95, units='rad')", color: "text-[#2dd4bf]" },
  { text: "", color: "" },
  { text: "    def compute_action(self, obs: JointState, dt: float = 0.01) -> np.ndarray:", color: "text-[#facc15]" },
  { text: "        pitch, roll = obs['imu_euler'][1], obs['imu_euler'][0]  # body orientation", color: "text-[#67e8f9]" },
  { text: "        torque = self.pd_control(pitch, roll, self.kp, self.kd, damping_ratio=0.7)", color: "text-[#a78bfa]" },
  { text: "        return np.clip(torque, -self.max_torque, self.max_torque).astype(np.float32)", color: "text-[#38bdf8]" },
  { text: "", color: "" },
  { text: "robot = G1Robot(connection='mujoco_sim', dof=29, control_freq=500, sensor_freq=1000)", color: "text-[#fb923c]" },
  { text: "skill = BalanceSkill(robot_model='g1_29dof', sim_dt=0.002, decimation=10)", color: "text-[#f97316]" },
  { text: "factory = SkillFactory(env='browser', physics='mujoco', gpu=True, num_envs=4096)", color: "text-[#fb923c]" },
  { text: "factory.train(robot, skill, episodes=2_000_000, lr=3e-4, batch_size=4096, epochs=5)", color: "text-[#e879f9]" },
  { text: "factory.export_onnx(skill, path='balance_v2.onnx', opset=17, optimize=True)", color: "text-[#c084fc]" },
  { text: "factory.deploy(robot, skill, target='production', verify=True, rollback_on_fail=True)", color: "text-[#4ade80]" },
  { text: "# Skill deployed successfully — 29 joints active, latency 1.2ms, reward 4850.3 ✓", color: "text-[#22c55e]" },
];
