...
const CODE_LINES = [
  { text: "import mujoco, numpy as np, torch", color: "text-purple-400" },
  { text: "from aurosy.skills import SkillFactory, deploy_config", color: "text-purple-400" },
  { text: "from unitree_sdk2.g1 import G1Robot, JointState", color: "text-purple-400" },
  { text: "", color: "" },
  { text: "class BalanceSkill(SkillFactory.BaseSkill):", color: "text-cyan-400" },
  { text: "    def __init__(self, robot_model='g1_29dof'):", color: "text-yellow-300" },
  { text: "        self.kp, self.kd = 120.0, 10.0  # PD gains", color: "text-green-300" },
  { text: "        self.target_height = 0.75  # meters from ground", color: "text-green-300" },
  { text: "        self.joint_limits = robot_model.get_limits()", color: "text-green-300" },
  { text: "", color: "" },
  { text: "    def compute_action(self, obs: JointState) -> np.array:", color: "text-yellow-300" },
  { text: "        pitch, roll = obs['imu_euler'][1], obs['imu_euler'][0]", color: "text-gray-300" },
  { text: "        torque = self.pd_control(pitch, roll, self.kp, self.kd)", color: "text-gray-300" },
  { text: "        return np.clip(torque, -self.max_torque, self.max_torque)", color: "text-cyan-300" },
  { text: "", color: "" },
  { text: "robot = G1Robot(connection='mujoco_sim', dof=29)", color: "text-orange-300" },
  { text: "skill = BalanceSkill(robot_model='g1_29dof')", color: "text-orange-300" },
  { text: "factory = SkillFactory(env='browser', physics='mujoco')", color: "text-orange-300" },
  { text: "factory.simulate(robot, skill, episodes=1000, render=True)", color: "text-purple-300" },
  { text: "factory.export_onnx(skill, path='balance_v2.onnx')", color: "text-purple-300" },
  { text: "factory.deploy(robot, skill, target='production')", color: "text-green-400" },
  { text: "# Skill deployed successfully — 29 joints active ✓", color: "text-green-500" },
];
