...
- **Создать движение** — предпросмотр траектории в окне: теперь возможен через NLA timeline (`hands/legs/torso`, Bezier handles, track weights), после чего выполняется семплирование и сглаживание для preview; без `mj_step` и без усилий на актуаторах.
...
| NLA-кривые и семплирование | [`nlaTimeline.ts`](../web/frontend/src/lib/nlaTimeline.ts), [`nlaEvaluation.ts`](../web/frontend/src/lib/nlaEvaluation.ts) |
